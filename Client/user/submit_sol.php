				...
				console.log(data);
				 if(data.compilation) {
						...
						if(data.result.run_status == "AC"){
							...
							$.ajax({
								 type: "get",
								 url: "set_problem_time.php?team_id=<?php echo $_COOKIE['team_id'];?>&problem_id=<?php echo $_GET['problem_id'];?>",
								 dataType: "json",
								 success: function(data){
									console.log(data);
									if(!data.status)
									{
										var timer_value=data.time;
										timer_value=parseInt(timer_value);
										var timeStamp = Math.floor(Date.now() / 1000);
										var left_time=timeStamp-timer_value;
									}
									else
									{
										alert('no data');
